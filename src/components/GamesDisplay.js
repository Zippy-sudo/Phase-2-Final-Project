...
function GamesDisplay({ gamesToDisplay, HandleShowMoreClick, HandleSaveClick, HandleDeleteClick, HandleShowLessClick }) {
  ...
  const gamesList = gamesToDisplay.map(game => {
    ...
    return <GameCard key={uuidv4()} id={id} game_url={game_url} developer={developer} release_date={release_date} freetogame_profile_url={freetogame_profile_url} title={title} thumbnail={thumbnail} platform={platform} publisher={publisher} genre={genre} short_description={short_description} HandleSaveClick={HandleSaveClick} HandleDeleteClick={HandleDeleteClick} />
  })
